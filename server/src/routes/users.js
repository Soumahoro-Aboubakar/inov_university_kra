...
router.patch('/:id/certification', async (req, res) => { const data = parse(z.object({ certified: z.boolean() }), req.body); const user = await User.findOneAndUpdate({ _id: req.params.id, role: { $in: ['local_doctor', 'contract_doctor'] } }, { certified: data.certified }, { new: true }).select('firstName lastName role certified'); if (!user) return res.status(404).json({ message: 'Docteur introuvable.' }); res.json(user); });
